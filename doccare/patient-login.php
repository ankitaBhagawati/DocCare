...
<?php
   include("conne.php");
//    //retrive any data alredy stored in the db
    if(isset($_POST['login'])){
   if($_SERVER["REQUEST_METHOD"] == "POST")//global var holds info about headers,paths and script location
    {
      $myemail = mysqli_real_escape_string($conn,$_POST['email']); //sql injection to remove/escape/sanitize unneccesary characters! secuurity
      $mypassword = mysqli_real_escape_string($conn,$_POST['password']);
    //   $patientid = mysqli_real_escape_string($conn,$_POST['patient_id']);
	  $sql="select * from `useraccount` where email = '$myemail'   and `registration_status` = '1' and role='patient'";
	  $result=mysqli_query($conn,$sql); //shows the sql query
	  $count = mysqli_num_rows($result); //count var to run if else
	   if($count>0) //if the query works
	   {    
...
// echo"$abc";
        //    setcookie('patient_id',$row['user_id'],time()+60*60*24*30);
                ...
                <div class="form-border border rounded-3 p-3 border-muted col-lg-9 shadow">
                    ...
                    <div class="form-group form-check">
                        ...
                        <label class="form-check-label" for="exampleCheck1">Remember Me.</label>
                            ...
                            <input type="submit" class="btn m-1 btn-primary col-lg-12" value="Log In" name="login">
